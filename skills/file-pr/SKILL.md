...
> Generated by [exact model name] via [harness].

Use the exact model identifier for the current session, including its spelling,
casing, and version. Do not use the harness name, provider name, model family,
or a shorthand alias. For example: `gpt-5.6-sol`, `muse spark 1.3`, or
`deepseek v4 flash`. Never guess the model name. If the exact identifier is
unavailable, ask before filing the PR.
