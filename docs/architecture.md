...
| `internal/trackedset`          | The tracked set of versioned custom-skill homes (env override, then explicit list order, then fleet-home slots alphabetical): the only reader of the ordering (`List`/`CollectionDirs`/`Resolve`) and the only writer of the explicit list (`Remember`/`Forget`). |
...
| `internal/snapshot`            | The read-side picture (rows × harness columns) that `ls` and the TUI both render. Unions canonical + tracked collections + fallback; precedence explicit-list order, then fleet-home checkouts alphabetically, then fallback, then canonical.                                                                                                         |
