...
`doctor.Analyze` runs the same link classification and config reads as sync, but writes nothing. Findings are grouped (redundant links, broken symlinks, unknown entries, manual edits fleet can't manage, state drift, missing directories, unreadable configs). Manual-edit _conflicts_ — where config and state disagree and both are expressible — are prompts: `keep` records the edit in the state file, `restore` re-projects the recorded intent. Everything else is reported for sync to fix or the user to handle.
