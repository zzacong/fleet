...
| `internal/paths`               | Every filesystem location, derived from one injected home root. `FLEET_HOME` overrides home. Exposes `FleetConfigFile()`, `FleetHomeSkills()`, `FleetReposDir()`, `InsideFleetHome()`. No other package calls `os.UserHomeDir()`.                                                                                                                       |
| `internal/trackedset`          | The tracked set of versioned custom-skill homes (env override, then explicit list order, then fleet-home slots alphabetical): the only reader of the ordering (`List`/`CollectionDirs`/`Resolve`) and the only writer of the explicit list (`Remember`/`Forget`). The skill index, pull, and drop call in instead of re-deriving it.                    |
| `internal/config`              | The machine-local config file `~/.config/fleet/config.json` (beside `state.json`): load/save, `skillsRepos` explicit repo-root list (order significant) plus `adoptTarget` collection dir, atomic write, unknown-field preservation (the retired `skillsRepo` key loads as an unknown and is never interpreted), validation for `fleet config` verbs.   |
| `internal/scan`                | Read-only listing of the canonical store, every tracked collection, the fallback, plus the skills CLI lockfile (provenance, sanitized dir names, frontmatter parsing).                                                                                                                                                                                  |
...
| `internal/doctor`              | Read-only report of drift, links, and manual edits; keep-or-restore resolution for conflicts; double presence across canonical, tracked collections, and fallback; a custom skill's managed link on Cursor/Bob disagreeing with the state; unscanned-adopt-target and non-git-repo warnings.                                                            |
...
| `internal/pull`                | The one new seam: git behind an injected `Runner` (real runner shells out with inherited stdio; tests inject a stub). Clone-into-slot vs remember-in-list (via the tracked set), same-remote fast-forward-only updates, per-repo reporting for bare pull.                                                                                               |
...
| `internal/snapshot`            | The read-side picture (rows × harness columns) that `ls` and the TUI both render. Unions canonical + tracked collections + fallback through the skill index; precedence explicit-list order, then fleet-home checkouts alphabetically, then fallback, then canonical.                                                                                   |
| `internal/skillindex`          | The skill index: every skill source scanned once in precedence order (tracked collections, fallback, canonical store), keyed by skill directory with frontmatter-name lookup. Snapshot, doctor, adopt, and update read through it; each keeps its own collision policy.                                                                                 |
...
| `internal/cli`, `internal/tui` | Faces. Cobra verbs (`skill ls/on/off/adopt/pull/update/sync/doctor`, `harness ls`, `config get/set/unset/list`) and the Bubble Tea matrix over the same core; neither keeps its own copy of any business logic.                                                                                                                                         |
...
`doctor.Analyze` runs the same link classification and config reads as sync, but writes nothing. Findings are grouped (redundant links, broken symlinks, unknown entries, manual edits fleet can't manage, state drift — including a custom skill's managed link on Cursor/Bob disagreeing with the state — double presence across canonical / tracked collections / fallback, stale lock entries for adopted skills, unscanned adopt target, non-git explicit repos, missing directories, unreadable configs). Manual-edit _conflicts_ — where config and state disagree and both are expressible — are reported with their keep/restore options by default; `fleet skill doctor -i` walks each one as a prompt: `keep` records the edit in the state file, `restore` re-projects the recorded intent. Everything else is reported for sync to fix or the user to handle.
