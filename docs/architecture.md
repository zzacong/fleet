...
| `internal/paths`               | Every filesystem location, derived from one injected home root plus the tracked set. `FLEET_HOME` overrides home. Exposes `FleetConfigFile()`, `FleetHomeSkills()`, `FleetReposDir()`, `TrackedRepos()`, `InsideFleetHome()`. No other package calls `os.UserHomeDir()`.                                                                              |
...
| `internal/pull`                | The one new seam: git behind an injected `Runner` (real runner shells out with inherited stdio; tests inject a stub). Clone-into-slot vs append-to-list write rule, same-remote fast-forward-only updates, per-repo reporting for bare pull.                                                                                                          |
